...
/* GET /api/v1/weather?city=X
   GET /api/v1/weather?lat=X&lon=X&name=Y&widget_id=X */
...
// --- Coordonnées fournies directement → skip géocodage ---
...
$cur     = $weather['current'];
$daily   = $weather['daily'];
$curInfo = $wmo[(int)$cur['weathercode']] ?? ['Inconnu', '🌡'];

$days = [];
for ($i = 0; $i < min(5, count($daily['time'])); $i++) {
    $info   = $wmo[(int)$daily['weathercode'][$i]] ?? ['Inconnu', '🌡️'];
    $days[] = [
        'date'   => $daily['time'][$i],
